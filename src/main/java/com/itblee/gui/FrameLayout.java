...
public class FrameLayout extends MovableJFrame {
	...
	private void initMenuButton() {
		...
		if (General.role.getQuyenHD() != null &&
				General.role.getQuyenHD().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenSP() != null &&
				General.role.getQuyenSP().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenPN() != null &&
				General.role.getQuyenPN().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenNCC() != null &&
				General.role.getQuyenNCC().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenKH() != null &&
				General.role.getQuyenKH().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenKM() != null &&
				General.role.getQuyenKM().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenTK() != null &&
				General.role.getQuyenTK().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenExcel() != null &&
				General.role.getQuyenExcel().getQuyenDoc() == 1) {
		...
		if (General.role.getQuyenNV() != null &&
				General.role.getQuyenNV().getQuyenDoc() == 1) {
	...
	private void initComponents() {
		...
		menuPanel = new JPanel();
		{
			...
			settingPanel = new JPanel();
			{
				...
				cbThemeChooser = new JComboBox<>(themeList);
				cbThemeChooser.setSelectedIndex(Theme.getIndexOf(currentTheme));
				cbThemeChooser.addItemListener(e -> {
				...
				List<Language> languageList = Language.getList();
				...
				cbLanguageChooser = new JComboBox<>(languageNameList);
				cbLanguageChooser.setSelectedItem(currentLanguage.getName());
				cbLanguageChooser.addActionListener(e -> onSelectedItemCBLanguageChooser((languageNameList[cbLanguageChooser.getSelectedIndex()])));
				...
				btnFontChooser.addActionListener(e -> onClickBtnFontChooser());
				...
				btnSaveSetting.addActionListener(e -> onClickBtnSaveSetting());
				...
				btnCancelSetting.addActionListener(e -> onClickBtnCancelSetting());
			...
			//---- infoPanel ----
			{
				...
				info.setText("Mã NV: NV" + General.user.getMaNV() + "\n\n" +
						"Mã TK: TK" + General.user.getMaTK() + "\n\n" +
						"Họ tên: " + General.user.getHoTen() + "\n\n" +
						"Ngày sinh: " + formatter.format(General.user.getNgaySinh()) + "\n\n" +
						"SDT: " + General.user.getSdt() + "\n\n" +
						"Email: " + General.user.getEmail() + "\n\n" +
						"Giới tính: " + (General.user.getGioiTinh() == 1 ? "Nam" : "Nữ"));
				...
				btnBackInfo.addActionListener(e -> onClickLBAvatar());
				...
				btnChangePassord.addActionListener(e -> onClickChangePassword());
			...
			btnContact.addActionListener(e -> onClickBtnContact());
			...
			btnLogout.addActionListener(e -> onClickBtnLogout());
			...
			btnSetting.addActionListener(e -> onClickBtnSetting());
			btnSetting.addMouseListener(new MouseAdapter() {
				@Override
				public void mouseEntered(MouseEvent e) {
					btnSetting.setContentAreaFilled(true);
					btnSetting.setIcon(
							createImageIcon("bin/images/components/setting-hover.gif" ,
									btnSettingSize*2, btnSettingSize*2)
					);
				}
				@Override
				public void mouseExited(MouseEvent e) {
					if (!settingPanel.isVisible()) {
						btnSetting.setContentAreaFilled(false);
						btnSetting.setIcon(
								createImageIcon("bin/images/components/setting.png" ,
										btnSettingSize, btnSettingSize)
						);
					}
				}
			});
	...
	private void onClickBtnLogout() {
		int confirm = JOptionPane.showConfirmDialog(
				getContentPane(),
				Language.LAYOUT_MESSAGE_LOGOUT,
				Language.LAYOUT_LABEL_LOGOUT_TOOL_TIP_TEXT,
		...
		if (result == JOptionPane.OK_OPTION) {
			try {
				...
				for (TaiKhoan dto : taiKhoanService.findAll()) {
					String hashPassword = Encryptor.applySha256(oldP, dto.getMatKhauSalt());
					if (dto.getMaTK().equals(General.user.getMaTK())
				...
				String newSalt = Encryptor.getSalt();
				String newHash = Encryptor.applySha256(oldP, newSalt);
				findTaiKhoan.setMatKhauSalt(newSalt);
				findTaiKhoan.setMatKhauHash(newHash);
				taiKhoanService.update(findTaiKhoan);
	...
	private void setSelectedButton(JButton newBtn) {
		if (newBtn == null)
			return;
		ArrayList<JButton> oldBtnList = new ArrayList<>();
