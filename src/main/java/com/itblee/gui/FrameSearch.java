...
public class FrameSearch extends MovableJFrame {
    ...
    public FrameSearch(String target, SearchMapper mapper, Class<?> clazz) throws Exception {
    ...
    private void onClickBtnSearchListener() {
        ...
        List<BaseEntity> searchList = mapper.searchAll();
    ...
    private List<BaseEntity> union(List<BaseEntity> list1, List<BaseEntity> list2) {
        List<BaseEntity> list = new ArrayList<>(list1);
        for (BaseEntity t : list2)
            if(!list.contains(t))
                list.add(t);
        return list;
    }
