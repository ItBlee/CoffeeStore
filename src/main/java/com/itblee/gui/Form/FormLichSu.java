...
import com.itblee.service.impl.AbstractService;
...
public class FormLichSu extends JPanel {
    ...
    private void loadBoards() {
        List<LichSu> list = lichSuService.findAll();
        for (int i = 1; i <= 10; i++) {
            try {
    ...
    private void initComponents() {
        ...
        jScrollBar.addAdjustmentListener(ae -> {
    ...
    public void createHistoryBoard(LichSu dto) {
        ...
        switch (dto.getThaoTac()) {
            case AbstractService.SAVE_FLAG:
                actionMark.setBackground(MyColor.GREEN);
                break;

            case AbstractService.UPDATE_FLAG:
                actionMark.setBackground(MyColor.ORANGE);
                break;

            case AbstractService.DELETE_FLAG:
        ...
        /*NhanVienServiceImpl nhanVienBUS = new NhanVienServiceImpl();
        NhanVien nhanVienDTO = nhanVienBUS.findByMaTK(dto.getNguoiThucHien());
